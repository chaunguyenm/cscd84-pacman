...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[PAC] = allPositions.copy()
    variableDomainsDict[GHOST0] = allPositions.copy()
    variableDomainsDict[GHOST1] = allPositions.copy()
    variableDomainsDict[OBS0] = range(X_RANGE + Y_RANGE - 1 + MAX_NOISE)
    variableDomainsDict[OBS1] = range(X_RANGE + Y_RANGE - 1 + MAX_NOISE)
    "*** END YOUR CODE HERE ***"
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for eliminationVariable in eliminationOrder:            
            ...
            if len(joinedFactor.unconditionedVariables()) != 1:
                eliminatedJoinedFactor = eliminate(joinedFactor, eliminationVariable)
                currentFactorsList.append(eliminatedJoinedFactor)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        times = int(self.numParticles / len(self.legalPositions))
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in beliefs:
            beliefs[ghostPosition] = beliefs[ghostPosition] * self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition)
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in beliefs:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for newPos in newPosDist:
                newBeliefs[newPos] = newBeliefs[newPos] + newPosDist[newPos] * beliefs[oldPos]
        newBeliefs.normalize()
        newParticles = []
        for i in range(self.numParticles):
